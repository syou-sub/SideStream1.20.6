    ...
    public void onEvent(Event<?> event) {
        ...
        if (event instanceof EventUpdate) {
            ...
            if(target != null  && target instanceof PlayerEntity){
                Scoreboard scoreboard = Objects.requireNonNull(mc.world).getScoreboard();
                ...
                if(scoreboardObjective != null) {
                   ...
                   ReadableScoreboardScore score = scoreboard.getScore(ScoreHolder.fromProfile(gameProfile), scoreboardObjective);
                    int targetHealth= Objects.requireNonNull(score).getScore();
                    int bestItemIndex = -1;
                    if (targetHealth <=5 && !did) {
                        current = Objects.requireNonNull(mc.player).getInventory().selectedSlot;
                        for (int b1 = 0; b1 < 9; b1++) {
                            ItemStack itemStack = mc.player.getInventory().getStack(b1);
                            if (itemStack == null) {
                                continue;
                            }
                            if (itemStack.getItem() == Items.RED_DYE && itemStack.getName().toString().contains("READY")) {
                                bestItemIndex = b1;
                            }
                        }
                        if (bestItemIndex != -1) {
        ...
        for (Entity entity : mc.world.getEntities()) {
            if (entity instanceof LivingEntity && entity != mc.player) {
                ...
                if (!PlayerHelper.fov(entity, fov.value))
                    continue;
                double focusRange = mc.player.canSee(entity) ? range.value : 3.5;
