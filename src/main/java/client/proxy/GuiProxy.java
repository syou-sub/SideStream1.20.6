...
public class GuiProxy extends Screen {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float partialTicks) {
        ...
        context.drawTextWithShadow(this.textRenderer, "Proxy Type", this.width / 2 - 150, positionY[1] + 5, 10526880);
        context.drawCenteredTextWithShadow(this.textRenderer, "Auth", this.width / 2, positionY[3] + 8, Formatting.WHITE.getColorValue());
        context.drawTextWithShadow(this.textRenderer, "IP:Port", this.width / 2 - 150, positionY[2] + 5, 10526880);

        this.ipPort.render(context, mouseX, mouseY, partialTicks);
        if (isSocks4) {
            context.drawTextWithShadow(this.textRenderer, "Auth ID", this.width / 2 - 150, positionY[4] + 5, 10526880);
            this.username.render(context, mouseX, mouseY, partialTicks);
        } else {
            context.drawTextWithShadow(this.textRenderer, "Auth Password", this.width / 2 - 150, positionY[5] + 5, 10526880);
            context.drawTextWithShadow(this.textRenderer, "Auth Username", this.width / 2 - 150, positionY[4] + 5, 10526880);
            this.username.render(context, mouseX, mouseY, partialTicks);
...
//        MinecraftClient.getInstance().keyboard.setRepeatEvents(true);
        ...
        ButtonWidget apply = ButtonWidget.builder(Text.of("Apply"), button -> {
            if (checkProxy()) {
        ...
        CheckboxWidget.Builder checkboxBuilder = CheckboxWidget.builder(Text.of("Proxy Enabled"), this.textRenderer);
        checkboxBuilder.pos((this.width / 2) - (15 + textRenderer.getWidth(Text.of("Proxy Enabled"))) / 2, positionY[7]);
