...
import static client.features.modules.render.HUD2.namebackground;
import static client.features.modules.render.HUD2.nameinfo;
...
public class HUD2 {
    ...
    public void draw(DrawContext context) {
        ...
        switch (client.features.modules.render.HUD2.namecolormode.getMode()) {
            case "Default":
      ...
      String     build = "Build: \2477" + Client.VERSION;
        String blockps = "Blocks/s: \2477" + String.valueOf(MathUtils.round(getDistTraveled(), 2));
      int nameX = 3;
      int nameY = 4;
      ...
      //  name = name.substring(0, 1).replaceAll(name.substring(0, 1), "\247c" + name.substring(0, 1)) + name.substring(1).replaceAll(name.substring(1), "\247f" + name.substring(1));
        ...
        if(nameinfo.enable){
            name = name + " " +String.format("[%s] [%dFPS]", new Object[] { time.substring(0,5), mc.getCurrentFps()});
        }
            Fonts.titleFont.drawString(context.getMatrices(), name, nameX,nameY, color);
        if(namebackground.isEnable()){
            context.fill(nameX,nameY,(int)Fonts.titleFont.getStringWidth(name)+5,(int)Fonts.titleFont.getFontHeight()+5,Colors.getColor(0,0,0,50));
        }
            if (client.features.modules.render.HUD2.info.isEnable()) {
                Fonts.font.drawString(blockps, 3, scaledResolution.getScaledHeight() - height, -1);
            }
            Fonts.font.drawString(build, 5, 16, -1);


        if(client.features.modules.render.HUD2.armor.isEnable()) {
            drawArmorStatus(scaledResolution,context);
        }
        if(client.features.modules.render.HUD2.effects.isEnable()) {
    ...
    private void drawGaeHud(DrawContext context,Window scaledResolution) {
        ...
        for (int i = 0, sortedListSize = sortedList.size(); i < sortedListSize; i++) {
            ...
            if (visible) {
                int color = -1;
                switch (client.features.modules.render.HUD2.colormode.getMode()) {
                    case "Default":
                ...
                Fonts.font.drawString(moduleLabel, (float) translateX, (float) translateY , color);
                      if ((Boolean) client.features.modules.render.HUD2.OUTLINE.enable) {
                    RenderingUtils.drawRect(context,translateX - 2.6D, translateY - 1.0D, translateX - 2.0D, translateY + listOffset - 1.0D, color);
                ...
                if ((Boolean) client.features.modules.render.HUD2.background.enable)
                    context.fill((int) (translateX - 2.0D), (int) (translateY - 1.0D), width, (int) (translateY + listOffset - 1.0D), Colors.getColor(0,0,0,50));
    ...
    public double getDistTraveled() {
        double total = 0;
        for (double d : client.features.modules.render.HUD2.distances) {
            total += d;
        }
        return total;
    }
    private void drawPotionStatus(Window sr) {
        ...
        for (StatusEffectInstance effect : collection) {
            ...
            Fonts.font.drawString(name, sr.getScaledWidth() - Fonts.font.getStringWidth(name + PType) - 3, sr.getScaledHeight() - 9 + pY, -1);
            Fonts.font.drawString(PType, sr.getScaledWidth() - Fonts.font.getStringWidth(PType) - 2, sr.getScaledHeight() - 9 + pY, -1);
            pY -= 9;
        }
    }
    private void drawArmorStatus(Window scaledRes, DrawContext context) {
        ...
        if (!mc.player.isCreative()) {
            ...
            for (int index = 3; index >= 0; index--) {
                ...
                if (stack != null) {
                    context.drawItem(stack,scaledRes.getScaledWidth() / 2 + x - 1, scaledRes.getScaledHeight() - (mc.player.isInsideWaterOrBubbleColumn() ? 70 : 50) - 2);
                    context.drawItem( stack, scaledRes.getScaledWidth() / 2 + x - 1, scaledRes.getScaledHeight() - (mc.player.isInsideWaterOrBubbleColumn() ? 70 : 50) - 2);
                    x += 18;
                }
            }
